...
extension MUXSDKStatsTests {
    ...
    func assertDispatchedCustomerDataEventsMatch(
        ...
        guard MockedData.dispatcher.dispatchedEvents.count > index else {
            XCTFail()
            return
        }
        XCTAssert(MockedData.dispatcher.dispatchedEvents.count > index)

        let dispatchedEvent = try XCTUnwrap(
            MockedData.dispatcher.dispatchedEvents[index].event as? MUXSDKDataEvent
        )
    ...
    func assertDispatchedCustomerViewerDataEventsAtIndex(
        index: Int,
        expectedCustomerViewerData: String
    ) {
        guard MockedData.dispatcher.dispatchedGlobalDataEvents.count > index else {
            XCTFail()
            return
        }
